...
Individu& VinsEtFromages::getIndividus(string nom, string prenom) const
{
...
void VinsEtFromages::afficherPresents() const {
	...
	cout << "# Entreprises presentes ( " << nombreIndividusDansCategorie(entreprise) << endl;
	for(unsigned int i=0;i<listeIndividus_.size();++i)
	{
		if (typeid(listeIndividus_[i]).name() == "Entreprise")
		cout << "# - " << listeIndividus_[i]->getString() << endl;
	}
	cout << "# Professeurs presents :" << nombreIndividusDansCategorie(professeur) << endl;
	for(unsigned int i=0;i<listeIndividus_.size();++i)
	{
		if (typeid(listeIndividus_[i]).name() == "Professeur")
		cout << "# - " << listeIndividus_[i]->getString() << endl;
	}
	cout << "# Etudiants presents :" << nombreIndividusDansCategorie(etudiant) << endl;
	for(unsigned int i=0;i<listeIndividus_.size();++i)
	{
		if (typeid(listeIndividus_[i]).name() == "Etudiant")
		cout << "# - " << listeIndividus_[i]->getString() << endl;
	}
}
...
int VinsEtFromages::nombreIndividusDansCategorie(Individu& individu) const
{
	int nbIndividus = 0;
		for(unsigned int i=0;i<listeIndividus_.size();++i)
	{
		if (typeid(listeIndividus_[i]).name() == typeid(individu).name())
			nbIndividus++;
	}
		return nbIndividus;
}